...
export interface SubscriptionMirrorInput {
  societyId: string;
  slug: string;
  /** Plan tier as recorded on the platform society document. */
  tier: string;
  /** Entitled module keys. Omitted/empty falls back to the tier's default set. */
  modules?: string[] | null;
  status?: string;
...
/**
 * Mirror the platform's entitlements into the society's own database.
 *
 * `requireModule` gates every feature on `enabledModules`, which the authenticate middleware
 * builds from the *tenant* `subscriptions` row — deliberately, so authorising a request never
 * costs a cross-database read. That makes the row a cache of platform truth, and a cache that
 * is only written once goes stale: a society upgraded in the Super Admin panel would keep being
 * refused the modules it now pays for (and a downgraded one would keep the modules it dropped).
 *
 * So this is an upsert, not an insert-if-absent, and every entitlement change re-runs it.
 * Fields the platform does not own (local status transitions, billing dates already set) are
 * left alone unless explicitly supplied.
 */
export async function syncSubscriptionMirror(db: TenantDatabase, input: SubscriptionMirrorInput): Promise<void> {
  const subscriptions = db.collection('subscriptions');
  const now = new Date();
  const tier = input.tier || 'STANDARD';
  const modules = input.modules?.length
    ? input.modules
    : DEFAULT_TIER_MODULES[tier] ?? DEFAULT_TIER_MODULES.STANDARD;

  const patch: Document = {
    societyId: input.societyId,
    planId: input.planId ?? `plan_${tier.toLowerCase()}`,
    planCode: tier,
    tier,
    modules,
  ...
  if (input.limits) patch.limits = input.limits;
  if (input.startDate) patch.startDate = input.startDate;
  if (input.endDate) patch.endDate = input.endDate;

  const existing = await subscriptions.findOne({ societyId: input.societyId });
  if (existing) {
    await subscriptions.updateOne({ societyId: input.societyId }, { $set: patch });
    return;
  }
  ...
  await subscriptions.create({
    ...
    startDate: patch.startDate ?? now,
    endDate: patch.endDate ?? new Date(now.getTime() + 14 * 86_400_000),
 ...
 * (The subscription mirror is the exception — it is platform-owned and is refreshed.)
 */
export async function seedTenantBasics(db: TenantDatabase, society: SeedTenantInput): Promise<void> {
  ...
  await syncSubscriptionMirror(db, {
    societyId,
    slug: society.slug,
    tier: society.planTier ?? 'STANDARD',
    modules: society.modules,
  });
