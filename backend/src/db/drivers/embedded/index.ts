...
import { getCollectionForScope, NEVER_SERIALISE } from '../../registry/index.js';
...
export class EmbeddedStore {
  ...
  /**
   * Serialises flushes. The debounce timer, `databases.flush()` and `close()` can all fire at
   * once; without this they would interleave and two writers would race on the same temp file.
   */
   ...
   *
   * Calls are chained rather than run concurrently: two overlapping flushes of the same
   * collection would otherwise write the same temp path, and whichever renamed second would fail
   * with ENOENT because the first had already moved the file out from under it.
   */
  flush(): Promise<void> {
    if (this.flushTimer) {
    ...
    // A failure must not poison the chain for later flushes.
    ...
    try {
      ...
      await Promise.all(
        names.map(async (name) => {
          ...
          const file = path.join(this.dir, `${name}.json`);
          // Unique per attempt so a retried or overlapping flush cannot collide on the temp name.
          this.tmpSeq += 1;
          const tmp = `${file}.${process.pid}.${this.tmpSeq}.tmp`;
          try {
            // Write-then-rename keeps the on-disk file either fully old or fully new.
...
export class EmbeddedDatabase implements TenantDatabase {
  ...
  constructor(
    readonly name: string,
    private readonly store: EmbeddedStore,
    /** Platform databases validate against the platform registry, tenants against the tenant one. */
  ...
  collection<T extends Document = Document>(collectionName: string): Collection<T> {
    ...
    const def = getCollectionForScope(this.scope, collectionName);
  ...
  hasCollection(collectionName: string): boolean {
    try {
      getCollectionForScope(this.scope, collectionName);
      return true;
    } catch {
      return false;
    }
...
export function createEmbeddedDatabase(
  ...
  return new EmbeddedDatabase(dbName, store, scope);
}
