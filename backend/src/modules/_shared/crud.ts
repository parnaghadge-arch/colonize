...
export interface CrudConfig<TCreate = unknown, TUpdate = unknown> {
  ...
  unitScoped?: boolean;
  /**
   * Column that ties a row to a unit. Defaults to `unitId`.
   *
   * Some collections reference the unit under another name (`units` scopes on its own `_id`,
   * `parking_slots` on `assignedUnitId`), and society-wide collections — amenities, gates, the
   * building/wing/floor directory, the vendor and staff rosters — have no unit column at all.
   * Those must set `unitScoped: false`: filtering on a column the collection does not have
   * matches nothing, so a resident silently gets an empty list. An empty amenity catalogue means
   * nobody in the society can book anything, and the failure looks like missing data rather than
   * an authorisation bug.
   */
  unitScopeField?: string;
...
export function buildCrudRouter<TCreate = unknown, TUpdate = unknown>(config: CrudConfig<TCreate, TUpdate>): Router {
  ...
  const scopeField = config.unitScopeField ?? 'unitId';
  ...
  function baseFilter(ctx: RequestContext, query: Record<string, unknown>): Document {
    ...
    return scopedFilter(ctx, { ...extra, ...fromList }, { unitScoped, unitScopeField: scopeField });
  }
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (unitScoped && ctx.principal.isResidentScope) Object.assign(filter, unitScopeFilter(ctx, scopeField));
...
export async function loadScoped(
  ...
  if (opts.unitScoped !== false && ctx.principal.isResidentScope) {
    Object.assign(filter, unitScopeFilter(ctx, opts.unitScopeField ?? 'unitId'));
  }
