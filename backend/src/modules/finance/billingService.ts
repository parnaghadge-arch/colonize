...
import { postBillAccrual, type AccountingContext } from './accountingService.js';
import { generateInvoicePdf, amountInWords } from '../../services/receiptPdf.js';
...
/**
 * Raise a one-off bill for a single unit with explicit lines (§28).
 *
 * Used for repair charges, fines or festival contributions that do not come from the monthly
 * generation run. It goes through exactly the same invariants as bulk generation — invoice
 * number from the counter, unique (unit, period), arrears carry-forward, the receivable accrual
 * in the ledger and the unit's outstanding balance — so a manual bill can never drift from the
 * books the way a hand-written insert would.
 */
export async function createBill(ctx: BillingContext, input: CreateBillInput): Promise<Document> {
  const settings = await billingSettings(ctx);
  const { start, end } = periodBounds(input.period);
  ...
  if (unit.isActive === false) throw ApiError.conflict('This unit is not active');
  ...
  if (existing) throw ApiError.conflict(`A bill for ${input.period} already exists for this unit (${existing.invoiceNumber})`);

  if (!input.items?.length) throw ApiError.badRequest('A bill needs at least one charge line');

  // Tax is computed per line so the invoice can show a real breakup (§59 GST billing).
  let order = 0;
  const lines = input.items.map((item) => {
    const amount = round2(Number(item.amount));
    const isTaxable = Boolean(item.isTaxable);
    const taxPercent = isTaxable ? Number(item.taxPercent ?? 0) : 0;
    return {
      type: String(item.type),
      label: String(item.label),
      amount,
      quantity: Number(item.quantity ?? 1),
      rate: item.rate === undefined ? null : round2(Number(item.rate)),
      isTaxable,
      taxPercent,
      taxType: isTaxable ? String(item.taxType ?? 'GST') : 'NONE',
      taxAmount: isTaxable ? round2((amount * taxPercent) / 100) : 0,
      order: order++,
    };
  });

  const lineTotal = round2(lines.reduce((sum, l) => sum + l.amount, 0));
  const totalTax = round2(lines.reduce((sum, l) => sum + l.taxAmount, 0));
  const taxableAmount = round2(lines.filter((l) => l.isTaxable).reduce((sum, l) => sum + l.amount, 0));
  const discount = round2(Math.max(0, Number(input.discount ?? 0)));
  if (discount > lineTotal) throw ApiError.badRequest('The discount cannot exceed the charge total');
  ...
  if (input.carryForwardArrears !== false) {
    ...
    arrears = round2(rows.reduce((sum, b) => sum + Number(b.dueAmount ?? 0), 0));
    if (arrears > 0) {
      lines.push({
        type: 'ARREARS',
        label: `Previous dues (${rows.length} bill${rows.length === 1 ? '' : 's'})`,
        amount: arrears,
        quantity: 1,
        rate: null,
        isTaxable: false,
  ...
  const subtotal = round2(lineTotal + arrears);
  const totalAmount = round2(subtotal + totalTax - discount);
  if (totalAmount <= 0) throw ApiError.badRequest('The bill total must be greater than zero');
  ...
  const invoiceNumber = await nextReference({ db: ctx.db, societyId: ctx.societyId, kind: 'INVOICE' });
  const billId = newId('maintenance_bills');
  const dueDate = new Date(input.dueDate as string | Date);
  ...
  await ctx.db.withTransaction(async () => {
    await ctx.db.collection('maintenance_bills').create({
      ...
      generatedAt: new Date(),
      dueDate,
      status: input.sendNotification === false ? 'GENERATED' : 'SENT',
      subtotal,
      discount,
      discountReason: discount > 0 ? String(input.discountReason ?? 'Discount applied') : null,
      taxBreakup: { cgst: 0, sgst: 0, igst: 0, cess: 0, taxableAmount },
      totalTax,
      arrears,
      penalty: 0,
      lateFee: 0,
      ...
      lastPaymentAt: null,
      paidAt: null,
      sentAt: input.sendNotification === false ? null : new Date(),
      reminderCount: 0,
      lastReminderAt: null,
      overdueSince: null,
      pdfUrl: null,
      ...
      configSnapshot: {
        ...
        dueDayOfMonth: settings.dueDayOfMonth,
        area: Number(unit.carpetAreaSqft ?? 0),
        manualItems: input.items.length,
        arrearsCarriedFrom: arrearsRows.map((b) => b.invoiceNumber),
      },
      createdBy: ctx.actorId,
      updatedBy: ctx.actorId,
    });
    ...
    // The receivable exists the moment the bill is raised, so the ledger always agrees with
    // what residents have been asked to pay.
    await postBillAccrual(ctx, { billId, amount: totalAmount, unitId: String(unit._id), invoiceNumber, period: input.period });
    return billId;
  });
  ...
  logger.info({ societyId: ctx.societyId, billId, invoiceNumber, unitId: unit._id, totalAmount }, 'bill created (manual)');
  return getBillDetail(ctx, billId);
}

/**
 * Render a bill as a tax invoice PDF (§59).
 *
 * Built server-side from the stored bill and its lines — including the rate snapshot the bill
 * was raised with — so the document a resident downloads can never disagree with the ledger.
 */
export async function renderInvoicePdf(ctx: BillingContext, billId: string): Promise<{ buffer: Buffer; filename: string; invoiceNumber: string }> {
  const detail = await getBillDetail(ctx, billId);
  ...
  const items = ((detail.items as Document[]) ?? []).map((i) => ({
    label: String(i.label),
    type: String(i.type),
    amount: Number(i.amount),
    quantity: Number(i.quantity ?? 1),
    rate: i.rate === null || i.rate === undefined ? null : Number(i.rate),
  }));
  ...
  const buffer = await generateInvoicePdf({
    ...
    society: {
      ...
      gstin: society?.gstin ? String(society.gstin) : null,
      logoUrl: society?.logoUrl ? String(society.logoUrl) : null,
    },
    unit: {
      label: String(detail.unit?.label ?? '—'),
      unitNumber: detail.unit?.unitNumber ? String(detail.unit.unitNumber) : undefined,
      building: detail.unit?.building ? String(detail.unit.building) : null,
      carpetAreaSqft: detail.unit?.carpetAreaSqft === null || detail.unit?.carpetAreaSqft === undefined ? null : Number(detail.unit.carpetAreaSqft),
    },
    billedTo: detail.billedTo
      ? {
          fullName: String(detail.billedTo.fullName),
          kind: detail.billedTo.kind ? String(detail.billedTo.kind) : undefined,
          phone: detail.billedTo.phone ? String(detail.billedTo.phone) : null,
        }
      : null,
    period: String(detail.period),
    periodStart: new Date(detail.periodStart as string | Date),
    periodEnd: new Date(detail.periodEnd as string | Date),
    dueDate: new Date(detail.dueDate as string | Date),
    items,
    subtotal: Number(detail.subtotal ?? 0),
    arrears: Number(detail.arrears ?? 0),
    lateFee: Number(detail.lateFee ?? 0),
    penalty: Number(detail.penalty ?? 0),
    discount: Number(detail.discount ?? 0) + Number(detail.waivedAmount ?? 0),
    waivedAmount: Number(detail.waivedAmount ?? 0),
    totalTax: Number(detail.totalTax ?? 0),
    taxBreakup: detail.taxBreakup ?? null,
    ...
    amountInWords: amountInWords(Number(detail.dueAmount ?? detail.totalAmount ?? 0)),
    generatedAt: new Date(),
  });

  return { buffer, filename: `${detail.invoiceNumber}.pdf`, invoiceNumber: String(detail.invoiceNumber) };
}
