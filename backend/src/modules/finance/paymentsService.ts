...
import { generateReceiptPdf, amountInWords } from '../../services/receiptPdf.js';
...
export interface IntentInput {
  ...
  /** Free amount for donations/fines; ignored when a linked record is supplied. */
  amount?: number;
  unitId?: string;
  provider?: string;
...
export async function createIntent(ctx: PaymentsContext, input: IntentInput): Promise<Document> {
  ...
  let event: Document | null = null;
  let serviceRequest: Document | null = null;
  ...
  if (input.billId) {
    ...
    event = await ctx.db.collection('events').findOne({ societyId: ctx.societyId, _id: input.eventId });
    if (!event) throw ApiError.notFound('Event');
    if (String(event.status) !== 'PUBLISHED') throw ApiError.conflict('This event is not open for registration');
    if (event.registrationDeadline && new Date(event.registrationDeadline as string | Date) < new Date()) {
      throw ApiError.conflict('Registration for this event has closed');
    }
    amount = Number(event.fee ?? 0);
    if (amount <= 0) throw ApiError.conflict('This event is free — there is nothing to pay');
    // The caller's own flat is the registration unit; never an id supplied by the client.
    unitId = ctx.isResidentScope ? ctx.unitIds?.[0] ?? null : input.unitId ?? null;
    if (!unitId) throw ApiError.badRequest('No flat is linked to your account to register this event against');
  } else if (input.serviceRequestId) {
    serviceRequest = await ctx.db.collection('service_requests').findOne({ societyId: ctx.societyId, _id: input.serviceRequestId });
    if (!serviceRequest) throw ApiError.notFound('Service request');
    if (serviceRequest.isBillable === false) throw ApiError.conflict('This service request is not billable');
    if (serviceRequest.billId) throw ApiError.conflict('This service request has already been billed');
    if (!['COMPLETED', 'CLOSED'].includes(String(serviceRequest.status))) {
      throw ApiError.conflict('A service request can only be charged once the work is completed');
    }
    // Charge what the work actually cost; fall back to the estimate the resident agreed to.
    amount = Number(serviceRequest.actualCharges ?? serviceRequest.estimatedCharges ?? 0);
    unitId = String(serviceRequest.unitId);
  } else {
  ...
  const payment = await ctx.db.collection('payments').create({
    ...
    referenceNumber,
    unitId,
    residentId: bill?.residentId ?? booking?.residentId ?? serviceRequest?.residentId ?? null,
    userId: ctx.actorId,
    purpose: input.purpose,
    status: 'INITIATED',
...
export async function applySuccessfulPayment(
  ...
  /* ---------------------------------- event ---------------------------------- */
    ...
    const event = await ctx.db.collection('events').findOne({ societyId: ctx.societyId, _id: payment.eventId });
    const cash = await accounting.ensureLedger(accountCtx, 'Bank / Cash', { type: 'ASSET', group: 'ASSET', code: 'ACC-1000' });
    const income = await accounting.ensureLedger(accountCtx, 'Event Income', { type: 'EVENT_INCOME', group: 'INCOME', code: 'INC-4004' });
    const entry = await accounting.postJournalEntry(accountCtx, {
      date: now,
      narration: `Event fee receipt ${receiptNumber}${event ? ` — ${event.name}` : ''}`,
      referenceType: 'RECEIPT',
      referenceId: String(payment._id),
      paymentId: String(payment._id),
      unitId: payment.unitId ? String(payment.unitId) : null,
      lines: [
        { ledgerId: String(cash._id), type: 'DEBIT', amount, note: 'Event fee received' },
        { ledgerId: String(income._id), type: 'CREDIT', amount, note: 'Event income' },
      ],
    });
    ...
    if (event) {
      await ctx.db.collection('events').updateOne({ societyId: ctx.societyId, _id: event._id }, { $inc: { collectedAmount: amount } });
      // One registration per flat per event (unique index) — paying confirms attendance.
      await ctx.db.collection('event_registrations').updateOne(
        { societyId: ctx.societyId, eventId: event._id, unitId: payment.unitId },
        {
          $set: { status: 'ATTENDING', paymentId: payment._id, amountPaid: amount },
          $setOnInsert: {
            _id: newId('event_registrations'),
            societyId: ctx.societyId,
            eventId: event._id,
            unitId: payment.unitId,
            residentId: payment.residentId ?? null,
            userId: payment.userId ?? ctx.actorId,
            guests: 0,
            note: null,
            passId: null,
            checkedInAt: null,
            clientRequestId: payment.clientRequestId ?? null,
            createdBy: ctx.actorId,
          },
        },
        { upsert: true },
      ...
      await ctx.db.collection('events').updateOne(
        { societyId: ctx.societyId, _id: event._id },
        { $inc: { registeredCount: 1, attendingCount: 1 } },
      );
    }
    ...
    await NotificationService.send({
      ...
      societyId: ctx.societyId,
      type: 'PAYMENT_SUCCESS',
      audience: { type: 'USERS', userIds: [String(payment.userId ?? ctx.actorId)] },
      data: { receiptNumber, amount, purpose: 'EVENT_FEE', eventId: payment.eventId, eventName: event?.name ?? null },
      deepLink: `/events/${payment.eventId}`,
    });
  }

  /* ------------------------------ service request ---------------------------- */
  else if (payment.serviceRequestId) {
    const request = await ctx.db.collection('service_requests').findOne({ societyId: ctx.societyId, _id: payment.serviceRequestId });
    const cash = await accounting.ensureLedger(accountCtx, 'Bank / Cash', { type: 'ASSET', group: 'ASSET', code: 'ACC-1000' });
    const income = await accounting.ensureLedger(accountCtx, 'Service Charge Income', { type: 'OTHER_INCOME', group: 'INCOME', code: 'INC-4005' });
    const entry = await accounting.postJournalEntry(accountCtx, {
      date: now,
      narration: `Service charge receipt ${receiptNumber}${request ? ` — ${request.title}` : ''}`,
      referenceType: 'RECEIPT',
      referenceId: String(payment._id),
      paymentId: String(payment._id),
      unitId: payment.unitId ? String(payment.unitId) : null,
      lines: [
        { ledgerId: String(cash._id), type: 'DEBIT', amount, note: 'Service charge received' },
        { ledgerId: String(income._id), type: 'CREDIT', amount, note: 'Service charge income' },
      ],
    });
    await ctx.db.collection('payments').updateOne({ societyId: ctx.societyId, _id: payment._id }, { $set: { journalEntryId: entry._id, ledgerId: income._id } });
    if (request) {
      await ctx.db.collection('service_requests').updateOne(
        { societyId: ctx.societyId, _id: request._id },
        { $set: { status: 'CLOSED' }, $push: { timeline: { at: now, action: 'CHARGE_PAID', note: `Paid ₹${amount} — receipt ${receiptNumber}`, by: ctx.actorId } } },
      );
    }
    await NotificationService.send({
      db: ctx.db,
      ...
      type: 'PAYMENT_SUCCESS',
      audience: { type: 'USERS', userIds: [String(payment.userId ?? ctx.actorId)] },
      data: { receiptNumber, amount, purpose: 'SERVICE_CHARGE', serviceRequestId: payment.serviceRequestId, title: request?.title ?? null },
      deepLink: `/service-requests/${payment.serviceRequestId}`,
    });
  }
...
/** Re-exported so callers can reach it from the payments module too. */
