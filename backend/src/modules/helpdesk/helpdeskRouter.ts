...
complaintsRouter.get(
  ...
  validate(z.object({ status: z.string().trim().max(30).optional(), open: z.coerce.boolean().optional(), limit: z.coerce.number().int().min(1).max(100).default(25) })),
  asyncHandler(async (req, res) => {
    const c = requireTenantContext(req);
...
complaintsRouter.get(
  ...
  authenticate(),
  requirePermission('complaint:view'),
  validate(z.object({ days: z.coerce.number().int().min(1).max(365).default(30) })),
  asyncHandler(async (req, res) =>
...
workOrdersRouter.get(
  ...
  validate(z.object({ status: z.string().trim().max(30).optional(), open: z.coerce.boolean().default(true) })),
  asyncHandler(async (req, res) => {
    const c = requireTenantContext(req);
    const q = req.query as Record<string, string>;
