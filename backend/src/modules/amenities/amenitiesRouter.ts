...
amenitiesRouter.get(
  ...
  validate(z.object({ date: z.string().trim().regex(/^\d{4}-\d{2}-\d{2}$/) })),
...
amenityBookingsRouter.get(
  ...
  validate(z.object({ status: z.string().trim().max(30).optional(), upcoming: z.coerce.boolean().default(true), limit: z.coerce.number().int().min(1).max(100).default(25) })),
  asyncHandler(async (req, res) => {
    const c = requireTenantContext(req);
...
amenityBookingsRouter.get(
  ...
  validate(z.object({ amenityId: idSchema.optional(), from: z.string().trim().max(30).optional(), to: z.string().trim().max(30).optional() })),
  asyncHandler(async (req, res) => {
    const c = requireTenantContext(req);
    const q = req.query as Record<string, string>;
