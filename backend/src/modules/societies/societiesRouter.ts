...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Modules a society no longer pays for stop resolving on its very next request: the
    // authenticate middleware re-reads the society record (60s cache) and rebuilds
    // `enabledModules`, and requireModule rejects anything outside that set.
    await logSystemAudit(ctx.platformDatabase, {
      collection: 'platform_audit_logs',
      societyId,
      module: 'subscription',
