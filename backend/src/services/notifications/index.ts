...
export const NotificationService = {
  async send(input: SendInput): Promise<SendResult> {
    ...
    const title = firstNonBlank(input.title, interpolate(template.title, data), 'Notification');
    const message = firstNonBlank(
      input.message,
      interpolate(template.body, data),
      `You have a new ${String(type).replaceAll('_', ' ').toLowerCase()} notification.`,
    );
  ...
  /**
   * Fire a notification without letting its failure break the calling operation.
   *
   * Use where the business record is already committed and a person may be physically waiting —
   * a guard scanning a pass at the gate, a resident approving a visitor from their flat. The
   * entry/exit record is the product; the push notification is a courtesy. A delivery failure is
   * logged and the flow continues, rather than surfacing as a 500 that leaves the gate blocked
   * (and, worse, leaves the client retrying an operation that already succeeded).
   */
  async sendBestEffort(input: SendInput): Promise<SendResult> {
    try {
      return await NotificationService.send(input);
    } catch (err) {
      logger.warn(
        { err, societyId: input.societyId, type: input.type },
        'notifications: delivery failed; continuing without blocking the operation',
      );
      return { notificationIds: [], recipients: 0, channels: input.channels ?? [], queued: 0 };
    }
  },
...
async function resolveTemplate(
  ...
  const builtIn = BUILT_IN_TEMPLATES[type as NotificationEvent];
  if (!builtIn) {
    // An event emitted with no template anywhere is a code/contract gap, not a user error.
    // It still sends (see `firstNonBlank`), but it should be visible in the logs.
    logger.warn({ societyId, type }, 'notifications: no template registered for event; using generic copy');
  }
  const fallback: NotificationTemplate = builtIn ?? {
    title: 'Notification',
 ...
 *
 * Returns a promise that resolves once every handler is actually registered. Callers must await
 * it *before* starting the queue: the queue drops any job whose name has no handler, so polling
 * first would silently discard the notifications enqueued during startup.
 */
export async function registerNotificationJobs(): Promise<void> {
  await Promise.all([registerPushJob(), registerSmsJob(), registerEmailJob(), registerWhatsAppJob()]);
}

function registerPushJob(): Promise<void> {
  // Imported lazily to avoid a cycle: queue ← notifications ← manager.
  return import('../../jobs/queue.js').then(({ registerJob }) => {
...
function registerSmsJob(): Promise<void> {
  return import('../../jobs/queue.js').then(({ registerJob }) => {
...
function registerEmailJob(): Promise<void> {
  return import('../../jobs/queue.js').then(({ registerJob }) => {
...
function registerWhatsAppJob(): Promise<void> {
  return import('../../jobs/queue.js').then(({ registerJob }) => {
...
/**
 * First candidate that renders as non-blank text.
 *
 * `notifications.title` and `notifications.message` are *required* columns, and the persistence
 * layer rejects an empty string as missing. A template gap — an event emitted with no matching
 * template, or a society override saved with a blank body — would otherwise turn a routine
 * business event (a gate check-in, a payment) into a 500 on that flow's critical path. Every
 * notification therefore always degrades to readable copy instead of failing the write.
 */
