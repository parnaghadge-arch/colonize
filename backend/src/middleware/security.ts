...
/**
 * Middleware that must run BEFORE the body parsers.
 *
 * `sanitiseInput` is deliberately not here: it rewrites `req.body`, so running it before
 * `express.json()` would sanitise nothing and leave the NoSQL-operator stripping silently
 * disabled. Use `afterBodyMiddleware()` once the body has been parsed.
 */
export function beforeBodyMiddleware() {
  return [requestId(), securityHeaders(), corsPolicy(), compression(), cookieParser()];
}

/** Middleware that must run AFTER the body parsers (it reads/rewrites `req.body`). */
export function afterBodyMiddleware() {
  return [csrfCookie(), sanitiseInput()];
}

/**
 * @deprecated Ordering trap — `sanitiseInput` runs before any body parser here, so request
 * bodies are never sanitised. Use `beforeBodyMiddleware()` + `afterBodyMiddleware()`.
 */
export function commonMiddleware() {
  return [...beforeBodyMiddleware(), ...afterBodyMiddleware()];
}
