...
export function authenticate(options: AuthenticateOptions = {}) {
  ...
  return async (req: Request, _res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      // The society always comes from the token — never from the request. But a client that
      // *names* a different society is either confused or probing, and quietly serving it this
      // token's tenant is how a user who manages two societies ends up writing society B's bills
      // into society A. The published contract for `x-society-id` promises a TENANT_MISMATCH here,
      // never a silent fallthrough, so refuse and make the client re-select its society.
      const requestedSocietyId = String(req.headers['x-society-id'] ?? req.query.societyId ?? '').trim();
      if (requestedSocietyId && requestedSocietyId !== String(societyDoc._id)) {
        logger.warn(
          { requestId, tokenSociety: String(societyDoc._id), requestedSociety: requestedSocietyId, userId: String(claims.sub) },
          'auth: rejected a request naming a society the token does not belong to',
        );
        throw ApiError.forbidden(
          'This session belongs to a different society. Sign in again to switch societies.',
          'TENANT_MISMATCH',
        );
      }
