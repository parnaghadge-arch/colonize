/**
 * API client for the Society Management Web App.
 *
 ...
 *
 * The base URL defaults to `/api` — a relative path — so the browser talks to its own origin
 * and the Vite dev server proxies to the backend. Nothing here ever points at localhost.
 */
...
export const tokenStore = {
  ...
  get societyId(): string | null {
    return localStorage.getItem(SOCIETY_KEY);
  },
  save(accessToken: string, societyId?: string | null): void {
    localStorage.setItem(TOKEN_KEY, accessToken);
    if (societyId) localStorage.setItem(SOCIETY_KEY, societyId);
  },
...
/** Trigger a browser download for a PDF endpoint (invoice, receipt). */
